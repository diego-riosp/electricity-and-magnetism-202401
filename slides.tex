...
\begin{frame}{Semana 13 (30/04/2024) - T12P1}
    ...
    \begin{figure}
        \centering
        \includegraphics[scale=0.25]{figures/t12p1.png}
    \end{figure}
...
\begin{frame}{Semana 13 (30/04/2024) - T12P2}
    ...
    \begin{figure}
        \centering
        \includegraphics[scale=0.4]{figures/t12p2.png}
    \end{figure}
...
\begin{frame}{Semana 13 (30/04/2024) - T12P3}
    
    In the figure, a bar magnet moves away from the solenoid.
    
    \begin{figure}
        \centering
        \includegraphics[scale=0.35]{figures/t12p3.png}
    \end{figure}
...
\begin{frame}{Semana 13 (30/04/2024) - T12P4}
    La corriente en el
alambre largo y recto $AB$ que se muestra en la figura va hacia arriba y se incrementa de forma constante a una razón $di/dt$.

\begin{figure}
        \centering
        \includegraphics[height=0.3\textwidth,width=0.4\textwidth]{figures/t12p4.png}
    \end{figure}
...
\item[d)] ¿Cuál es la fem inducida en la espira?
\end{itemize}
\end{frame}

\begin{frame}{Semana 13 (30/04/2024) - T12P5}
...
\begin{figure}
        \centering
        \includegraphics[height=0.3\textwidth,width=0.7\textwidth]{figures/t12p5.png}
    \end{figure}

\vspace{-1.5em}

La región del campo magnético es considerablemente más ancha que 50.0 cm. Calcule la magnitud y dirección (sentido horario o antihorario) de la corriente inducida en el circuito conforme está:
...
\begin{frame}{Semana 13 (30/04/2024) - T12P6}
    
    La corriente en la figura obedece la ecuación $I(t)=I_0e^{-bt}$, donde $b>0$.
    
    \begin{figure}
        \centering
        \includegraphics[scale=0.6]{figures/t12p6.png}
    \end{figure}
...
\begin{frame}{Semana 13 (30/04/2024) - T12P7}
     ...
     \begin{figure}
        \centering
        \includegraphics[scale=0.35]{figures/t12p7.png}
    \end{figure}
...
\begin{frame}{Semana 14 (07/05/2024) - T13P1}
    A square wire frame with side length $\ell$ has total resistance $R$. It
is being pulled with speed $v$ out of a region where there is a uniform $B$ field pointing out of the page (the shaded area in Fig.).
Consider the moment when the left corner is a distance $x$ inside
the shaded area.
...
\begin{figure}
        \centering
        \includegraphics[scale=0.35]{figures/t13p1.png}
    \end{figure}
    \column{0.5\textwidth}
    \begin{itemize}
        \item[a)] What force do you need to apply to the square so that it moves with constant speed $v$?
        \item[b)] Verify that the work you do down from $x=x_0$ (which you can assume is less than $\ell/\sqrt{2}$) to $x = 0$ equals the energy dissipated in the resistor.
    \end{itemize}
\end{columns}
...
\begin{frame}{Semana 14 (07/05/2024) - T13P2}
    Los campos magnéticos dentro de una mancha solar llegan a tener una intensidad tan grande como 0.4 T. (En comparación, el campo magnético terrestre es de alrededor de $110\,000$ de ese valor). Las manchas solares tienen hasta $25\,000$ km de radio. El material en una mancha solar tiene una densidad de cerca de $3 \times 10^{-4}$ kg/m$^3$. Suponga que $\mu$ para el material de la mancha solar es $\mu_0$. Si el 100\% de la energía del campo magnético almacenada en una mancha se pudiera usar para expulsar de la superficie del Sol el material de la mancha, ¿con qué rapidez sería expulsado el material?
Compare el resultado con la rapidez de escape del Sol, que es de aproximadamente $6 \times 10^5$ m/s. (\textit{Sugerencia:} Calcule la energía cinética que podría proveer el campo magnético a 1 m$^3$ de material de la mancha
solar)
\end{frame}

\begin{frame}{Semana 14 (07/05/2024) - T13P3}
\small
    Un conductor sólido y pequeño, con radio $a$, está sostenido por discos aislantes no magnéticos sobre el eje de un tubo de paredes delgadas con radio interior $b$. Los conductores interior y exterior transportan corrientes iguales $i$ en sentidos opuestos.
\begin{itemize}
    ...
    \item[b)] Escriba la expresión para el flujo $d\Phi_B$ a través de una banda angosta de longitud $l$ paralela al eje, de ancho $dr$, a una distancia $r$ del eje del cable, y que se encuentra en el plano que contiene al eje.
\item[c)]  Integre su expresión del inciso b) con respecto al volumen comprendido entre los dos conductores para determinar el flujo total producido por una corriente $i$ en el conductor central.
...
\begin{frame}{Semana 13 (30/04/2024) - T13P4}
\small
    Se han ignorado las variaciones del campo magnético en toda la sección transversal de un solenoide toroidal. Estudiemos la validez de ese método. Cierto solenoide toroidal tiene sección transversal rectangular (\textit{ver} figura). Tiene $N$ espiras espaciadas de manera uniforme, con aire en el interior. El campo magnético en un punto dentro del toroide está dado por la ecuación: $$B=\frac{\mu_0NI}{2\pi r}.$$
No suponga que el campo es uniforme en toda la sección transversal.
\begin{columns}
    \column{0.3\textwidth}
    \begin{figure}
        \centering
        \includegraphics[width=1\textwidth]{figures/t13p4.png}
    \end{figure}
    \column{0.7\textwidth}
...
%\footnotesize
    ...
    \item[a)] Demuestre que el flujo magnético a través de una sección transversal del toroide es $$\Phi_B=\frac{\mu_0NIh}{2\pi}\ln\left(\frac{b}{a}\right).$$
...
\begin{frame}{Semana 14 (07/05/2024) - Q6}
    ...
    \begin{figure}
        \centering
        \includegraphics[scale=0.4]{figures/q6.png}
    \end{figure}
...
\begin{frame}{Semana 15 (14/05/2024) - T14P1}
    
    En el circuito que se
muestra en la figura, determine la lectura en cada amperímetro y voltímetro

\begin{figure}
        \centering
        \includegraphics[scale=0.4]{figures/t15p1.png}
    \end{figure}

\begin{itemize}
    \item[a)] Justo después de haber cerrado el interruptor $S$
    \item[b)] Después de que S ha estado cerrado durante mucho tiempo
\end{itemize}

\end{frame}


\begin{frame}{Semana 15 (14/05/2024) - T14P2}
    
    En el circuito que se ilustra en la figura, ni la batería ni los inductores tienen una resistencia apreciable, los capacitores están inicialmente descargados, y el interruptor $S$ ha estado mucho tiempo en la posición 1.

\begin{figure}
        \centering
        \includegraphics[scale=0.4]{figures/t15p2.png}
    \end{figure}

\begin{itemize}
    \item[a)] ¿Cuál es la corriente en el circuito?
    \item[b)] Ahora se acciona repentinamente el interruptor a la posición 2. Calcule la carga máxima que recibirá cada capacitor y cuánto tiempo se requerirá para adquirir esa carga a partir del momento en que fue accionado el interruptor.
\end{itemize}

\end{frame}

\begin{frame}{Semana 15 (14/05/2024) - T14P3}
    
     En el circuito que se presenta en la figura, $\mathcal{E} =
60.0$ V, $R_1 = 40.0 \,\Omega$, $R_2 = 25.0 \,\Omega$
y $L = 0.300$ H.

\begin{figure}
        \centering
        \includegraphics[scale=0.3]{figures/t15p3.png}
    \end{figure}

El interruptor $S$
se cierra en $t = 0$. Inmediatamente
después de cerrar el interruptor,

\begin{itemize}
    \item[a)] ¿cuál es la diferencia de potencial
vab entre los extremos del resistor
$R_1$?
    \item[b)] ¿cuál punto, $a$ o $b$, está a un potencial más alto?
    \item[c)] ¿cuál es la diferencia de potencial $v_{cd}$ entre los
extremos del inductor $L$?
\item[d)] ¿Cuál punto, $c$ o $d$, está a un potencial más
alto?
\end{itemize}

\end{frame}

\begin{frame}{Semana 15 (14/05/2024) - T14P4}
    
    Dos bobinas están enrolladas una alrededor de la otra, como se ilustra en la figura. La corriente fluye en el mismo sentido alrededor de cada bobina. Una de ellas tiene autoinductancia $L_1$ y la otra una autoinductancia $L_2$. La inductancia mutua de las dos bobinas es $M$.

\begin{columns}
    \column{0.5\textwidth}
    \begin{figure}
        \centering
        \includegraphics[scale=0.3]{figures/t15p4.png}
    \end{figure}
    \begin{itemize}
        \item[a)] Demuestre que si las dos bobinas se conectan en serie, la inductan-
cia equivalente de la combinación es 
\end{itemize}
    \column{0.5\textwidth}
...
\item[b)] Demuestre
que si las dos bobinas están conectadas en paralelo, la inductancia
equivalente de la combinación es $$L_{\text{eq}}=\frac{L_1L_2-M^2}{L_1+L_2-2M}$$
    \end{itemize}
\end{columns}
